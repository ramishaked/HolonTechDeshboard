...
async function browserRun() {
  ...
  else if (fc.nRows !== fc.nHist + 3) fail(`תצוגה 14 — ${fc.nRows} מוטות במקום ${fc.nHist + 3}`);
  else if (!fc.ordered) fail('תצוגה 14 — שנות העבר אינן כרונולוגיות לפני השנה הנוכחית');
  else if (!fc.offOk) fail('תצוגה 14 — מוט הרשמי אינו תואם ל-holonSeries');
  else if (fc.prevPct != null && fc.expPrev != null && Math.abs(fc.prevPct - fc.expPrev) > 0.06)
    fail(`תצוגה 14 — מוט השנה הקודמת ${fc.prevPct.toFixed(1)} מול גולמי ${fc.expPrev.toFixed(1)}`);
  else pass(`תצוגה 14 — ${fc.nHist} שנות עבר + היום + צפי ${fc.y1}/${fc.y2} זכאים · ${fc.nSch} בתי ספר`);
  if (!fc.skip) {
    ...
    else if (!fd.offGone) fail('תצוגה 14 — מוט הרשמי (מכנה מצבת) מוצג גם על המכנה העירוני');
